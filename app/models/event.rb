class Event < ActiveRecord::Base
  ...
  validates :min_attendees, 
            :numericality => { :greater_than_or_equal_to => 0, 
                               :less_than_or_equal_to => :max_attendees, 
                               :message => "must be less than or equal to max attendees", 
                               :unless => lambda {|e| e.min_attendees.blank? or e.max_attendees.blank?} }, 
            :presence => true
  validates :max_attendees, 
            :numericality => { :greater_than => 0, 
                               :greater_than_or_equal_to => :min_attendees, 
                               :message => "must be greater than or equal to min attendees",
                               :unless => lambda {|e| e.min_attendees.blank? or e.max_attendees.blank?} }, 
            :presence => true
