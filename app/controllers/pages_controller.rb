class PagesController < ApplicationController
  ...
  def home_events
    ...
    if session[:sort_order] == 'distance'
      ...
      .where(%Q(#{Event.quoted_table_column(:id)} NOT IN (#{@scheduled_events_with_venue_tbd.select(Event.quoted_table_column(:id)).to_sql}))).limit(15)  #limit added because it was too slow. Pagination needed
      @events = @events.by_distance_from(@location)
    else
      @other_events = Event.where(%Q(#{Event.quoted_table_column(:id)} NOT IN (#{@events.select(Event.quoted_table_column(:id)).to_sql}))).limit(15) #limit added because it was too slow. Pagination needed
