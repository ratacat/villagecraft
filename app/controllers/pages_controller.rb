class PagesController < ApplicationController
  def home
    ...
    @workshops_with_upcoming_or_ongoing_reruns = Workshop.joins(:first_meetings).where(%Q(#{Meeting.quoted_table_column(:end_time)} > ?), Time.now).order(%Q(#{Meeting.quoted_table_column(:start_time)}))
